...
FIELDS = {"id", "dish", "restaurant", "year", "price", "price_today", "image_url"}
...
def check(items: list[dict]) -> list[str]:
    ...
    bad("missing/extra fields", [i for i in items if set(i) != FIELDS])
...
def main() -> None:
    ...
    if args.check_images:
        ...
        for it in rng.sample(items, args.check_images):
            req = urllib.request.Request(it["image_url"], method="HEAD",
                                         headers={"User-Agent": "Mozilla/5.0"})
            try:
                with urllib.request.urlopen(req, timeout=30) as r:
                    ok += r.headers.get("Content-Type", "").startswith("image/")
            except Exception:
                pass
        print(f"images resolving: {ok}/{args.check_images}\n")

    rng = random.Random(args.seed)
    print(f"-- random {args.sample} items to hand-check --")
    print(f"{'year':<6}{'price':>8}{'today':>11}  dish / restaurant")
    for it in sorted(rng.sample(items, args.sample), key=lambda i: i["year"]):
        print(f"{it['year']:<6}{it['price']:>8.2f}{it['price_today']:>11,.2f}  "
              f"{it['dish'][:40]:<42}{it['restaurant'][:34]}")
